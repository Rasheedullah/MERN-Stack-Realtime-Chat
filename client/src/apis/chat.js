...
export const removeUser = async (body) => {
  try {
    const token = localStorage.getItem('userToken');
    const { data } = await API(token).patch('/api/chat/removeFromGroup', body);
    return data;
  } catch (error) {
    console.log('error in remove user api');
  }
};
export const getNewGroups = async (body) => {
  try {
    const token = localStorage.getItem('userToken');
    const { data } = await API(token).get('/api/chat/getNewGroups', body);
    return data;
  } catch (error) {
    console.log('error in Get Groups api');
  }
};
